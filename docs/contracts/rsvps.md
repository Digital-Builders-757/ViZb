...
- Persists entitlement in **`event_registrations`** and issues a **`tickets`** row (16-char hex **`ticket_code`**) via **`mint_free_rsvp_ticket_for_registration`** ($0 completed **`orders`** + line item).
...
- RSVP + mint: `app/actions/registrations.ts`

  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-col gap-3">
      <div className="border-b border-[color:var(--neon-hairline)]/80 pb-3">
        <p className="font-serif text-lg font-bold text-[color:var(--neon-text0)] sm:text-xl">
          {formatDashboardDayPanelHeading(dayKey, sorted.length)}
        </p>
        <p className="mt-1 font-mono text-xs uppercase tracking-wider text-[color:var(--neon-text2)] sm:text-sm">
          Choose an event
        </p>
      </div>
        ...
        <ul className="min-w-0 space-y-2">
          {sorted.map((e) => (
            <li key={e.id} className="min-w-0">
              ...
              >
                <p className="break-words text-base font-semibold text-[color:var(--neon-text0)]">{e.title}</p>
                <p className="mt-1 font-mono text-sm text-[color:var(--neon-a)]">
                  {formatDashboardEventTimeShort(e.starts_at, e.ends_at)}
                </p>
                <p className="mt-1 break-words text-sm text-[color:var(--neon-text1)] sm:text-base">
                  {e.city}

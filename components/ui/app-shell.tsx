...
import { CausticBackdrop, type CausticVariant } from "@/components/ui/caustic-backdrop"
import { cn } from "@/lib/utils"

export interface AppShellProps extends React.HTMLAttributes<HTMLDivElement> {
  /** When true, adds aurora + noise on top of the global starfield. */
  withNeonBackdrop?: boolean
  /** Optional caustic atmosphere layer (CSS-only). */
  causticVariant?: CausticVariant | null
}
...
export function AppShell({
  ...
  return (
    <div className={cn("relative min-h-[100dvh] overflow-x-hidden", className)} {...props}>
      {causticVariant ? <CausticBackdrop variant={causticVariant} /> : null}

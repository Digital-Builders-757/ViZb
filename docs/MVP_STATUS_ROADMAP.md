...
- **June 16, 2026 — Eventbrite import pipeline (#259):** Server-only Eventbrite v3 client (**`lib/eventbrite/*`**), normalize + dedupe upsert (**`lib/imports/*`**), **`event_import_runs`** audit table + source columns on **`events`**; admin queue **`/admin/events/imports`** (approve/reject/bulk/edit); manual **`POST /api/admin/imports/eventbrite/run`**; cron **`GET /api/cron/eventbrite-import`** every 6h via **`vercel.json`**; gated by **`EVENTBRITE_IMPORT_ENABLED`**. See **`docs/imports/eventbrite.md`**.
- **June 22, 2026 — Event Import Review admin bridge (#270 partial):** **`/admin/events/imports`** is now a unified **Event Import Review** workspace over **`event_candidates`** (Ticketmaster + Eventbrite) with source tabs, filters, paginated queue, candidate detail + audit history, reject/suppress/link/duplicate actions, controlled **Publish** into community **`events`**, separate manual import controls, import run history, and source health. Legacy pre-#266 Eventbrite rows on **`events`** remain in a labeled section until drained.
- **June 16, 2026 — PWA install experience (#250):** **`app/manifest.ts`** (ViZb branding, standalone, 192/512/maskable icons under **`public/pwa/`**); root layout **`viewport`** + **`appleWebApp`** metadata; global **`PwaInstallPrompt`** with Chrome **`beforeinstallprompt`**, iOS Share → Add to Home Screen fallback, 10-day dismiss cooldown, and suppression on auth/checkout/RSVP paths. Helpers: **`lib/pwa/*`**, **`hooks/use-pwa-install-prompt.ts`**. No service worker (install-only). Verified: **`npm run typecheck`**, **`npm run test`** (317), **`npm run lint`**, **`npm run build`**.
- **June 16, 2026 — Branded PWA + favicon icons:** Replaced generic placeholder PWA/favicon assets with icons generated from canonical **`/vizb-logo.png`** (**`scripts/generate-pwa-icons.mjs`**, **`npm run icons:generate`**); **`favicon.ico`**, **`favicon-32x32.png`**, **`apple-icon.png`**, **`public/pwa/*`**; removed stale **`public/icon.svg`** / **`public/icon.png`**; icon paths centralized in **`lib/brand-assets.ts`**. Existing home-screen shortcuts must be removed and re-added after deploy. Verified: **`npm run typecheck`**, **`npm run test`** (317), **`npm run lint`**, **`npm run build`**.
- **June 16, 2026 — Dark-to-glow contrast pass (#254):** Removed site-wide gray haze from white **`--neon-surface`** / **`backdrop-blur`** stacking; retuned glass tokens to dark blue/cyan (**`app/globals.css`**); lighter caustic/editorial overlays; **`.events-card-surface`** for listing cards; event timeline/discovery card polish. Verified: **`npm run lint`**, **`npm run build`**.
- **June 16, 2026 — Member dashboard command center:** **`/dashboard`** rebuilt as a personal control room (not a discovery feed): **`DashboardCommandCenter`** (“What’s the move?”), **`MyNextMoveCard`**, planner (**Your week** / locked-in + saved rows), tickets/passes, **Saved but not decided**, vibe profile, compact **Local pulse** digest, and **Recommended for you** (≤6 cards) at the bottom. Data centralized in **`loadDashboardHome()`** + **`lib/dashboard/dashboard-home-types.ts`**; **`loadMemberHomeRsvpSummary`** exposes full upcoming ticket list + event IDs for planner dedupe. Legacy home rails removed from the page (components retained). **`PlannerSection`** nests under command-center stats (**Locked in** / **On your radar** counts) via embedded variant. Verified: **`npm run typecheck`**, **`npm run test`** (318), **`npm run lint`**, **`npm run build`**.
- **June 16, 2026 — Dashboard planner calendar restored:** Re-mounted **`DashboardCalendarShell`** on **`/dashboard`** (Month / Week / Agenda town calendar) after first-run onboarding; **`?cal=YYYY-MM`** month navigation; **`getPublishedEventsForDashboardMonth`** + **`fetchMySavedEventIds`** wired on the page; loading skeleton for calendar height; **`#my-vibes-week-heading`** anchor restored on embedded **`PlannerSection`** for event-detail deep links. Verified: **`npm run typecheck`**, **`npm run test`** (328), **`npm run lint`**, **`npm run build`**.
- **June 16, 2026 — Events discovery scroll simplification:** Public **`/events`** quick filters slimmed to **All events**, **Tonight**, **This weekend**, **My Vibes** + **Filters** sheet (Free, category, price presets moved sheet-only); hero **Jump to events** removed — **`EventsSearchBar`** with gradient border in **`EventsDiscoveryHero`**; **Recent past** archive grid removed from timeline (past listings remain at **`/events/[slug]`**). **Starting soon** / **ViZb picks** compact rails removed; **Staff picks** uses shared **`EventsFeaturedMoment`** neon scroll on **`/events`** + homepage. Timeline **Weekend wave**, **New this week**, and **City spotlight** inserts removed — scroll is date headers + cards only. Verified: **`npm run typecheck`**, **`npm run test`** (318), **`npm run lint`**, **`npm run build`**.
- **June 16, 2026 — Brighter gradient borders / glow (#252):** Token-level brighten pass in **`app/globals.css`** (**`--vibe-neon-glow*`**, **`--events-glow-*`**, **`--events-card-radial-*`**, **`.events-neon-card*`** utilities, timeline spine, **`water-border`**, **`GlassCard`** hover). Event discovery rails, timeline cards, filter chips, and flyer fallbacks wired to shared tokens. Verified: **`npm run typecheck`**, **`npm run test`**, **`npm run lint`**, **`npm run build`**.
- **June 16, 2026 — ViZB payments + Connect payouts (M1–M10):** Canonical launch pricing (**5% + $1/ticket**, processing passthrough, **$5** min paid tier) in **`lib/payments/vizb-pricing-config.ts`**; checkout preview aligned with Stripe three line items; order fee/payout columns + **`organizer_stripe_accounts`** + **`organizer_payouts`** migrations; hardened Stripe webhooks (refunds, disputes, idempotency); **Stripe Connect Express** organizer onboarding (**`/organizer/{slug}/payments`**); payout ledger + hourly **`/api/cron/release-payouts`**; staff **Payments review** (**`/admin/payments`**, payouts ledger, manual hold/release); internal record **`docs/vizb-payments-pricing-and-payouts.md`**. Verified: **`npm run typecheck`**, **`npm run test`** (299), **`npm run lint`**, **`npm run build`**.
...
**Still outstanding for Phase 4+:** Stripe Tax; automated partial-refund policy; post-release payout clawback automation; dedicated door UI polish (Phase 5)
...
| Attendee home page | `app/(dashboard)/dashboard/page.tsx` | DONE — command center: hero + next move + stats, embedded **Your week** planner + **Town calendar** (`DashboardCalendarShell`), tickets, saved-not-decided, vibe profile, local pulse, recommendations (discovery at bottom); first-run prefs + host/admin CTAs |
| Member planner calendar | `components/dashboard/calendar/*` | DONE — mounted on **`/dashboard`**: Month / Week / Agenda + Eastern dates; day + event selection; detail panel (desktop) / Sheet (mobile); ICS via `app/api/calendar/ics`; org “Hosted by”; query **`getPublishedEventsForDashboardMonth`** (widened window). Re-export: `dashboard-month-calendar.tsx` → shell. |
...
| Push notifications for event reminders | Medium | Medium | Web manifest + install prompt shipped (#250); service worker still required for push |

-- RSVP capacity for free events: optional limit on confirmed + checked_in registrations.
-- NULL rsvp_capacity = unlimited (backward compatible).
...
-- Safe occupancy count for published events only (for public detail page + server checks).
...
-- Enforce capacity at write time (prevents races vs application-only checks).

...
export function HomeEventsGrid({ data }: HomeEventsGridProps) {
  ...
  return (
    <section id="events" className="home-events-section relative w-full scroll-mt-24 px-0 py-14 sm:py-16 md:py-24">
      <div className="relative z-10 mx-auto max-w-7xl px-5 sm:px-8">
        <div className="mb-10 flex flex-col justify-between gap-6 md:mb-16 md:flex-row md:items-end">
          <div>
            <h2 className="mb-2 inline-flex items-center gap-2 text-sm font-bold uppercase tracking-normal text-[color:var(--neon-a)]">
              <Waves className="h-4 w-4" aria-hidden />
              Staff picks
            </h2>
            <h3 className="max-w-3xl text-balance text-4xl font-black leading-tight text-[color:var(--neon-text0)] md:text-5xl">
              Curated <span className="home-redesign-text-gradient">Experiences</span>
            </h3>
            <p className="mt-4 max-w-2xl text-pretty text-sm leading-relaxed text-[color:var(--neon-text1)] sm:text-base">
              A first look at what is moving now. Open the full timeline for filters, quick previews,
              saves, and the complete local map.
            </p>
          </div>
          <Link
            href="/events"
            className="group inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-[color:var(--neon-hairline)] bg-[color:var(--neon-surface)]/35 px-5 font-mono text-[10px] uppercase tracking-normal text-[color:var(--neon-text1)] transition-colors hover:border-[color:var(--neon-a)]/45 hover:text-[color:var(--neon-a)]"
          >
            View all events

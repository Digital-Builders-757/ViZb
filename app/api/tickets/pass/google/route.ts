...
import { isGoogleWalletPassConfigured } from "@/lib/tickets/pass-config"
...
export async function GET(request: NextRequest) {
  ...
  const rid = request.nextUrl.searchParams.get("rid")?.trim()
  if (!rid) {
  ...
  return NextResponse.json(
    { ok: true, message: "google pass not yet generated" },
    {
      status: 200,
      headers: {

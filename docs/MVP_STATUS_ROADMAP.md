...
- [x] **Organizer event edit — save scopes (April 2026)** — Event details section before RSVP/ticket tiers; **Save event details** (dirty-gated submit, sonner confirmation, sticky action bar, microcopy for whole-event cap vs tiers) vs per-row **Save tier**; `EventDetailsEditForm` remount via `updated_at` key on organizer + admin event pages

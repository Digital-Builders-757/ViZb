    ...
    <div className="border-t border-[color:var(--neon-hairline)]/50 bg-[color:var(--neon-surface)]/20 px-4 py-4 sm:px-5 sm:py-5">
      ...
      {open ? (
        <div id={panelId} className="mt-5 flex flex-col items-center gap-4">
          ...
          <p className="max-w-xs text-center font-mono text-[10px] leading-relaxed text-[color:var(--neon-text2)]">
            Staff can scan this QR, or you can read your{" "}
            <span className="text-[color:var(--neon-text1)]">backup code</span> aloud. Don&apos;t post screenshots
            publicly.
          </p>
          ...
          <div className="w-full max-w-sm space-y-2">
            ...
            <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-center">
              <code className="block min-h-[44px] w-full min-w-0 max-w-full flex-1 select-all overflow-hidden break-all rounded-lg border border-[color:var(--neon-hairline)] bg-[color:var(--neon-bg0)]/40 px-3 py-2.5 text-center font-mono text-[clamp(0.625rem,2.8vw,0.6875rem)] leading-relaxed text-[color:var(--neon-text0)] [overflow-wrap:anywhere] sm:text-left">
                {showFullBackupCode ? token.trim() : maskBackupCode(token)}
              </code>
              <button
                type="button"
                onClick={copyBackup}
                className="inline-flex min-h-[44px] w-full shrink-0 items-center justify-center rounded-full border border-[color:var(--neon-hairline)] bg-[color:var(--neon-surface)]/40 px-4 font-mono text-[10px] uppercase tracking-widest text-[color:var(--neon-text0)] transition-colors hover:border-[color:var(--neon-a)]/45 sm:w-auto"
              >
